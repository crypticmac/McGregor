...
/*
  please ignore the instructions in the doc for the moment. Gitian build will be integerated at a later stage. <a href=""></a>
*/


*Setup instructions for a Gitian build of McGregor using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the McGregor
...
- [Building McGregor](#building-McGregor)
...
Clone the git repositories for McGregor and Gitian and then checkout the McGregor version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/crypticmac/McGregor.git
cd McGregor
...
Follow the instructions in [doc/release-process.md](release-process.md) in the McGregor repository
...
To build McGregor (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the McGregor repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/McGregor/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/crypticmac/McGregor
...
For example:
```bash
URL=https://github.com/user/McGregor.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit McGregor=${COMMIT} --url McGregor=${URL} ../McGregor/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit McGregor=${COMMIT} --url McGregor=${URL} ../McGregor/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit McGregor=${COMMIT} --url McGregor=${URL} ../McGregor/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/McGregor-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/McGregor-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/McGregor-build.assert
```
...
[McGregor/gitian.sigs](******unavailable at the mo********) repository, or if that's not possible create a pull
request. 
